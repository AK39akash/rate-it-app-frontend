...
export default function UserDashboard({ view }) {
  ...
  async function fetchData() {
    ...
    //   FETCH RATINGS ONLY IF LOGGED IN
      if (token) {
  ...
  const myRatings = useMemo(() => {
    const map = {};
    ratings.forEach((r) => {
      if (r.userId === user.id) map[r.storeId] = r.value;
    });
    return map;
  }, [ratings, user.id]);
  ...
  return (
    <div className="pb-5">
      ...
      >
        ...
        {(!view || view === "ratings") && (
          <Row className="g-4">
            {stores
              .filter(
                (store) => !view || (view === "ratings" && myRatings[store.id]),
              )
              .map((store, i) => (
        ...
        {view === "settings" && (
          <div
            className="glass-pane p-3 p-md-4 w-100"
      ...
      >
        ...
        <Modal.Body>
            Please login to rate stores and submit reviews.
        </Modal.Body>
